...
const Home: React.FC = () => {
    ...
    useEffect(() => {
        const currentNetworkName = networkName || suggestedName || new Array(nameLength).fill('').map(() => nameCharacterSet.charAt(Math.floor(Math.random() * nameCharacterSet.length))).join('');
